...
\begin{equation}
					N_b= 4 \pi \sigma^2 \delta
\end{equation}

where $\delta$ is the density of adults per unit area and $\sigma$ is the variance in distance between birth and breeding sites. This formulation is often called the neighborhood size and assumes a normal distribution of distances between parents and offspring (out in a perfect circular shape from the source). Thus, changes in the variance of dispersal distance can affect $N_e$ (highly clumped populations will have reduced $N_e$). This is the basic model of "Isolation by Distance" proposed by   \citet{Wright:1943aa,Wright:1946aa}. Under this type of model, migration (gene flow) is given by the variance in dispersal, and not by the proportion of the population that is composed of migrants (denoted $m$), as is the case with island models \citep{Slatkin:1985qb}.
...
\begin{equation}
  			N_b = 4 \pi (\sigma^2_s + \frac{t \sigma^2_p}{2}) \delta (1 + t)
\end{equation}

where $\sigma^2_s$ is the variance in seed dispersal, $\sigma^2_s$ is the variance in pollen dispersal and $\delta$ is the density of potential parents.
...
The extent of neighborhood area in plants can be extremely variable, depending on life-history attributes such as life-span, spacing patterns, mating system, etc. Even a limited sample of available information (Table S1) highlights the fact that the size of neighborhood areas can in some cases exceed the geographic limits of local populations \citep{Nason:1998aa}. The size of neighborhood areas may encompass at least four orders of magnitude, $10{^-2}-10^2$ km in radius, and include many individuals. Therefore, reference to this "genetic/evolutionary" paradigm and reference \citep[sensu][]{Waples:2006ev} may be instrumental to understand the actual role of LDD events in shaping the structuring of genetic pools and contributing to gene dispersal. 

Whenever there is a large discrepancy between population area extent and $N_b$ we might expect the frequency of $LDD_{loc}$ and $LDD_{neigh}$ differ enormously. For example, relatively small $N_b$ may rise the importance of $LDD_{loc}$ in preserving scenarios of panmixia within a local population, as most distant dispersal events will disseminate seeds outside the neighborhood of maternal plants.
